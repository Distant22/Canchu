...
module.exports = {
    ...
    search: async(res,keyword) => {
        const sql = 'SELECT users.id, users.name, users.picture, friendship.id AS friend_id , friendship.status FROM users INNER JOIN friendship ON users.id = friendship.user_id WHERE users.name LIKE '+`'%${keyword}%'`
